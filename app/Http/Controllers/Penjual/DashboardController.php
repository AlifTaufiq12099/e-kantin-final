...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Rating - hitung dari transaksi yang selesai (dummy untuk sekarang, bisa dibuat tabel rating terpisah)
        // Untuk sementara, kita hitung berdasarkan jumlah transaksi selesai
        $totalTransaksiSelesai = Transaksi::where('lapak_id', $lapakId)
            ->where('status_transaksi', 'selesai')
            ->count();
        
        // Rating bisa dihitung dari jumlah transaksi atau dibuat tabel rating terpisah
        // Untuk sekarang, kita set default atau bisa dihitung dari feedback jika ada
        $rating = $totalTransaksiSelesai > 0 ? 4.5 : 0; // Default rating
        $totalUlasan = $totalTransaksiSelesai; // Jumlah transaksi selesai sebagai proxy untuk ulasan
